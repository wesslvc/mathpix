...
- 보기 표지(ㄱ/ㄴ/ㄷ) 렌더링에 두 가지 손질이 들어가 있다:
  ① Mathpix가 **조합용 자모**(U+1100~, `ᄀᄂᄃ`)로 주는 경우가 있는데, 이건
     음절 조립용 코드라 홀로 쓰면 위첨자처럼 아주 작게 그려져 눈에 띄게 깨지고
     `BOGI_ITEM`(U+3131~)에도 안 걸려 박스 감지 자체가 안 된다. `normalizeJamo()`가
     호환용 자모로 바꾼다. **유니코드 NFKC로는 안 된다** — 표준이 정한 방향이
     반대(ㄱ→ᄀ)라서 표를 직접 들고 있다.
  ② 호환용 자모라도 나눔명조에서는 음절보다 작게 그려진다(캔버스로 재보니 잉크
     높이가 0.696배, 밑변도 위로 뜬다). `.mmd-bogi-marker`가 **자음만** 1.30배로
     키우고 밑변을 맞춘다. 배율은 1.00/1.15/1.30/1.44를 나란히 렌더링해 눈으로
     고른 값이다(1.44는 표지가 본문을 눌러 과했다). 마침표는 span 밖이라 본문
     크기 그대로다 — 같이 키우면 점이 굵고 낮게 앉아 오히려 어색해진다.
- 앱 이름 "수학오답프린트 제작"
