...
-- 사용자 본인의 크레딧을 원자적으로 1 차감한다. 잔액이 없으면 아무 것도 갱신하지
-- 않는다(호출부에서 반환값이 없으면 "크레딧 부족"으로 판단). 첫 호출이면 무료
-- 크레딧 5개로 행을 만든 뒤 차감한다. auth.uid()만 건드릴 수 있어 다른 사용자
-- 크레딧은 조작할 수 없다.
...
-- 본인의 크레딧을 1 되돌려준다(Mathpix 호출 자체가 실패했을 때 환불용).
...
-- 지정한 사용자에게 크레딧을 더한다(결제 완료 웹훅 전용). service_role만 호출
