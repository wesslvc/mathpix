 ...
 */
const ERASE_HANDWRITING = `손으로 쓴 것은 하나도 남기지 마세요(가장 자주 실패하는 부분입니다):
- 연필·볼펜·형광펜·색연필로 쓴 모든 것을 지웁니다: 메모, 풀이 과정, 계산 자국,
  밑줄, 동그라미, 세모, 별표, 화살표, 체크, 빗금, 지운 자국, 수정액 자국,
  채점 표시, 정답 표시, 번호 옆에 친 표시.
- **검은색 볼펜으로 쓴 것도 손글씨입니다.** 색으로 판단하지 마세요. 손으로 쓴
  것은 획의 굵기가 들쭉날쭉하고, 글자 크기와 기울기가 제각각이며, 인쇄 글자와
  서체가 다르고, 줄이나 칸 밖으로 삐져나가고, 인쇄물 위에 겹쳐 있습니다.
- 옅게 남기거나 흐리게 뭉개지 말고 **완전히 없애세요.** 손글씨에 가려진 인쇄
  내용은 그 아래에 있던 대로 되살려 그리세요(지운 자리를 빈칸으로 두지 마세요).
- 다만 **인쇄된** 밑줄·굵은 글씨·강조는 원본의 일부이니 반드시 그대로 두세요
  (밑줄 친 낱말을 묻는 문제가 많습니다). 인쇄된 밑줄은 두께가 일정하고 글자와
  나란히 반듯합니다.
- 종이 접힌 자국, 얼룩, 그림자, 스캔 줄무늬, 손가락, 기울어짐도 없애고 똑바로
  펴 주세요.`;
