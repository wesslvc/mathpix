 ...
 * Llama 3.2 Vision 계열의 더 작은 11b로 내림. 한때 "완전히 다른 모델"로
 * microsoft/phi-3.5-vision-instruct를 시도했으나 그 모델은 실제로 이
 * API 카탈로그에 없는 것으로 확인돼(사용자가 build.nvidia.com에서 직접
 * 확인한 코드 스니펫으로 llama-3.2-11b-vision-instruct의 존재를 재확인함)
 * 다시 11b로 되돌림 — 같은 계정/키로 호출하는 무료 엔드포인트라 비용
 * 차이는 없음.)
 */
