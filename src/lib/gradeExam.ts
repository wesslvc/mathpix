...
- **객관식 표기 방식이 한 OMR 안에 두 가지로 섞여 있을 수 있습니다.**
  ① 문항 하나가 한 줄 전체를 차지하고, 그 줄에 문항 번호와 ①~⑤ 원이 가로로 함께 있는 방식.
  ② 문항 번호들이 **가로로 나열된 헤더**로 먼저 나오고, 그 아래 각 문항 번호의 **세로줄**에 ①~⑤ 원이
     위에서 아래로 늘어서 있는 격자(행렬) 방식 — 이 경우 문항 번호가 적힌 세로줄을 그대로 따라
     내려가며 그 줄에서 마킹된 원이 몇 번째인지로 답을 정하세요(가로줄 기준으로 읽으면 다른 문항의
     마킹을 그 문항 것으로 착각합니다). 문항 번호 자체는 인쇄된 라벨일 뿐 마킹이 아니니, 학생이 실제로
     칠한 원과 헷갈리지 마세요.
...
- slot 1의 items 개수는 1선택 정답표의 문항 수와, slot 2는 2선택 정답표의 문항 수와 같아야 합니다.`;
  ...
  // 수학은 단답형이 ①~⑤ 동그라미가 아니라 **숫자 자리별 격자**로 표기된다
  // (백의 자리·십의 자리·일의 자리 세 줄, 각 줄에 0~9가 원으로 늘어서 있고
  // 그중 하나씩 마킹). 이 형식을 설명해 주지 않으면 모델이 세 줄을 서로
  // 다른 문항으로 착각하거나, 마킹 안 된 자리를 무시해 자릿수를 통째로
  // 잘못 읽는다(사용자가 실제로 그렇게 헷갈린다고 신고했다) — 탐구·국어에는
  // 이 형식이 없으므로 수학에만 붙인다.
  const mathShortAnswer =
    subject === "math"
      ? `
수학에는 객관식(①~⑤ 원 하나에 마킹) 말고 **단답형**이 섞여 있습니다. 단답형은 원 마킹이 아니라
**숫자 자리별 격자**로 표기됩니다 — 문항 하나당 "백의 자리"·"십의 자리"·"일의 자리" 세 줄이 있고,
각 줄에 0~9 숫자가 원으로 늘어서 있어 그중 하나에 마킹합니다.
- **줄의 위치(위·아래)가 아니라 그 줄 옆에 적힌 "백"·"십"·"일" 글자를 보고 자리를 판단하세요** — 사진마다
  줄 순서가 다를 수 있습니다.
- 세 자리를 이어 붙인 숫자가 답입니다. 마킹이 없는 자리는 0으로 봅니다(앞자리 0은 생략하고 적으세요).
  예) 일=5, 십·백 마킹 없음 → 답 "5" · 일=8, 십=9, 백 마킹 없음 → 답 "98" · 일=0, 십=0, 백=1 → 답 "100"
- **세 줄 모두 마킹이 없을 때만 무마킹(null)**입니다. 한 자리라도 마킹이 있으면 나머지 빈 자리는 0으로
  채워 답을 만드세요 — 자리 하나가 비었다고 통째로 무마킹 처리하지 마세요.
`
      : "";
