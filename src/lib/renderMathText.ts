 ...
 * 때문에 별도 블록으로 갈린 경우가 정확히 이 모양이다). 이 조건을 만족할
 ...
 * 보이면(일반 텍스트 줄) 기존처럼 그 지점에서 잘라 뒤쪽을 박스 밖으로 빼다.
 ...
 * 높음) 박스를 "열어둔 채" 딥 한 블록만 더 넘어가 본다.
 *
 * 반대로 (가)/(나) 조건이 애당초 마침표 없는 순수 수식으로 끝나는 문제도
 * 있다(예: "(나) 2×sin(∠ECD)=3×sin(∠EDC)"). 이런 경우 다음 블록이 위 조건에
 * 안 맞으면(순수 수식 플레이스홀더가 아니라 "DF의 값은?" 같은 일반 본문이면)
 * 마침표를 찾겠다고 계속 뒤져서는 안 되므로 — 그럼 무관한 본문을 끝없이
