...
const DEFAULT_MODEL_IDS = [
  // 사용자가 지정한 모델. luna는 품질이 부족하다고 해서 terra로 올렸다.
  // 계정의 /v1/models 목록에는 있지만 **실제 호출로는 아직 검증되지 않았다** —
  // 목록에 있어도 못 부르는 경우가 이 프로젝트에서 여러 번 있었다. 배포 뒤
  // /api/figure/models 를 열어 probe가 ok인지 확인할 것.
  "gpt-5.6-terra",
  // 못 부를 때의 폴백. 2026-08 프로브에서 200을 확인한 이름들이고, 이 시점부터는
  // 비용보다 품질이 우선이라는 요구에 맞춰 좋은 것부터 늘어놓는다.
  "gpt-5",
  "gpt-4.1",
  "gpt-4o",
  "gpt-5-mini",
