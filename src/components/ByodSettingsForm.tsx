...
import { BYOD_IMAGE_MODEL_CHOICES } from "@/lib/figureImageGen";

/**
 * BYOD(Bring Your Own [OpenAI] Key) 패스 설정 — 본인 OpenAI 키 등록·교체·
 * 삭제, 이미지 생성 모델 선택.
 *
 * **BYOD 패스를 산 사람에게만 보인다**(`/profile`이 `access.byod`로 가른다).
 * 패스가 없는 사람에게 키 입력칸을 보여줘 봐야 아무 효과가 없다 — 서버
 * 라우트가 어차피 `entitlements.byod`를 다시 확인한다(화면 숨김은 우회할
 * 수 있으므로 진짜 게이트는 서버다).
 *
 * **키는 이 화면을 거쳐 한 번도 평문으로 우리 서버에 안 남는다** — 브라우저가
 * `set_byod_openai_key` RPC를 직접 불러 Postgres(Vault)에 곧바로 암호화해
 * 넣는다. 등록 뒤에는 값을 다시 안 보여준다(`has_key`만 안다) — 그래서
 * "키 교체"는 항상 새 값을 다시 입력해야 한다.
 */
export default function ByodSettingsForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase.rpc("get_byod_status").maybeSingle();
      if (cancelled) return;
      if (error) {
        setError(error.message);
      } else {
        const row = data as { byod: boolean; has_key: boolean; model: string | null } | null;
  ...
  async function saveKey() {
    ...
    try {
      const supabase = createClient();
      const { error } = await supabase.rpc("set_byod_openai_key", {
        p_key: keyInput.trim(),
        p_model: model || null,
      });
  ...
  async function saveModel(next: string) {
    ...
    try {
      const supabase = createClient();
      const { error } = await supabase.rpc("set_byod_model", { p_model: next || null });
      if (error) throw error;
      setSaved("모델을 바꿨어요.");
    } catch (err) {
  ...
  async function clearKey() {
    if (!confirm("등록한 OpenAI 키를 지울까요? 지우면 다시 넣기 전까지 BYOD 기능을 못 씁니다.")) {
      return;
    }
    setSaving(true);
    setSaved(null);
    ...
    try {
      const supabase = createClient();
      const { error } = await supabase.rpc("clear_byod_openai_key");
      if (error) throw error;
      setHasKey(false);
      setSaved("키를 지웠어요.");
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4">
      <div>
        <p className="text-sm font-medium text-slate-700">BYOD 패스 — 본인 OpenAI 키</p>
        ...
        <>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              ...
              {BYOD_IMAGE_MODEL_CHOICES.map((id) => (
