...
/**
 * 수학 표지에 **성명·수험번호 칸을 되살린다.**
 *
 * 원본 hwpx 에서 문항을 걷어낼 때 같이 떨어져 나갔다(탐구 표지에는 남아 있다).
 * 두 문제지는 같은 판형이고 이 칸도 같은 물건이라, 탐구 표지의 것을 그대로
 * 옮겨 온다. 다만 **`제 [ ] 선택` 은 탐구 전용**이라 빼고(수학에는 선택과목
 * 표기가 표지에 없다), 남은 두 칸을 종이 한가운데에 맞춘다.
 *
 * 세로 자리는 영역명 글줄을 기준으로 잡는다 — 두 표지의 영역명 높이가 6.5pt
 * 다르므로 절대 좌표를 그대로 쓰면 그만큼 어긋난다. 칸이 생기면 그 아래
 * 가로줄도 따라 내려가야 하고(탐구와 같은 9pt 간격), 가로줄에서 시작하는
 * **세로 단 구분선의 윗끝도 같이** 내려야 한다 — 안 그러면 구분선이 새 칸을
 * 뚫고 올라간다.
 */
function copyNameRow(src, dst, gapBelow) {
  const yOf = (i) =>
    i.k === "text" ? [i.y] : i.k === "line" ? [i.y1, i.y2] : [i.y, i.y + i.h];
  const xOf = (i) =>
    i.k === "text" ? [i.x] : i.k === "line" ? [i.x1, i.x2] : [i.x, i.x + i.w];
  // 성명·수험번호 칸이 놓인 띠. `제 [ ] 선택`(x≥520)과 아래 가로줄은 뺀다.
  const row = src.items.filter(
    (i) =>
      yOf(i).every((y) => y >= 155 && y <= 191) && xOf(i).every((x) => x < 520),
  );
  if (!row.length) return;

  const areaLine = (frame) =>
    frame.items.filter((i) => i.k === "text").sort((a, b) => b.size - a.size)[0].y;
  const dy = areaLine(dst) - areaLine(src);

  const lines = row.filter((i) => i.k === "line");
  const left = Math.min(...lines.flatMap(xOf));
  const right = Math.max(...lines.flatMap(xOf));
  const dx = dst.width / 2 - (left + right) / 2;

  const copied = row.map((i) => {
  ...
  const rule = dst.items.find(
    (i) => i.k === "line" && Math.abs(i.y1 - i.y2) < 0.5 && Math.abs(i.x2 - i.x1) > 600,
  );
  if (!rule) return;
  const bottom = Math.max(...copied.filter((i) => i.k === "line").flatMap(yOf));
  const ruleY = bottom + gapBelow;
  {
    for (const i of dst.items) {
      // 가로줄에 윗끝을 대고 있던 세로 구분선도 같이 내린다.
      if (i.k === "line" && Math.abs(i.x1 - i.x2) < 0.5 && Math.abs(i.y1 - i.y2) > 300) {
        if (Math.abs(Math.min(i.y1, i.y2) - rule.y1) < 1) {
          if (i.y1 < i.y2) i.y1 = ruleY;
          else i.y2 = ruleY;
        }
      }
    }
    rule.y1 = rule.y2 = ruleY;
  }
  dst.items.splice(dst.items.indexOf(rule), 0, ...copied);
}
copyNameRow(out.tamgu.first, out.math.first, 9);
